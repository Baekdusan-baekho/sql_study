...
-- WHERE employees.department_id = departments.department_id; 원래 형태
WHERE e.department_id = d.department_id; -- 오라클 조인
-- null 값은 데이터가 들어오지 않았음

-- 테이블은 두개 세개 네개 이렇게 조인할 수 있다.

SELECT
    e.first_name, e.last_name, e.hire_date,
    e.salary, e.job_id, d.department_name
FROM employees e INNER JOIN departments d -- 위에 것보다 가독성이 좋고 정확한 조인 지정
ON e.department_id = d.department_id; -- ANSI 표준 조인 문법
-- WHERE절과 FROM절을 구분해서 가독성이 좋게 한다.
...
SELECT
    ...
    d.department_name, e.job_id, j.job_title, loc.city,  loc.state_province
FROM 
    employees e,
    departments d,
    jobs j,
...
AND d.location_id = loc.location_id -- 순서2
AND loc.state_province = 'California'; -- 일반조건 where  역할/순서 1번

/*
1. loc 테이블의 state_province = 'California 조건이 맞는 값는 대상으로
2. location_id 값과 같은 값을 가지는 데이터를 departments에서 찾아서 조인
3. 위의 조인한 결과와 동일한 department_id를 가진 employees 테이블의 데이터를 찾아 조인
4. 위에 결과와 jobs테이블을 비교하여 조인하고 최종 결과를 출력
*/
...
/*
employees 테이블에는 존재하고, departments테이블에는 존재하지 않아도
