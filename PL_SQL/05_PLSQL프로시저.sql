...
CREATE PROCEDURE guguproc -- 프로시저
    (p_dan IN NUMBER)
IS --선언부  선언하지 않아도 됨
BEGIN  -- 실행부
...
EXEC my_new_job_proc('JOB2', 'test job', 2000, 3000);
...
BEGIN
    -- 외부에선 EXEC
    my_new_job_proc('JOB2', 'test3', 2000, 8000, msg);
    -- msg에 p_result 조회된 값을 집어넣는다
...
SELECT * FROM jobs;
-- '존재하지 않는 값이라 INSERT 처리 되었습니다.'; 안나옴 
----------------------------------
    ...
    */
    ...
    EXCEPTION
        WHEN ZERO_DIVIDE THEN
            dbms_output.put_line('"0"으로 나누시면 안돼요!');
