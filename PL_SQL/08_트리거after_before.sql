...
:OLD = 참조 전 열의 값 (INSERT: 입력 전 자료, UPDATE: 수정 전 자료, DELETE: 삭제될 값) 기존 값
:NEW = 참조 후 열의 값 (INSERT: 입력 할 자료, UPDATE: 수정 된 자료) 동작한 후의 값

테이블에 UPDATE나 DELETE를 시도하면 수정, 또는 삭제된 데이터를
별도의 테이블에 보관해 놓는 형식으로 트리거를 많이 사용합니다.

트리거 자체는 트랜잭션의 일부로 처리하기 때문에 COMMIT 이나 ROLLBACK을 포함 할 수 없습니다.
*/
...
DECLARE
    v_type VARCHAR2(10);
BEGIN
    -- 현재 트리거가 발동된 상황이 update 인지 delete 인지를 파악하는 조건문
...
BEGIN
    ...
    EXCEPTION 
        WHEN quantity_shortage_exception THEN 
            ...
            -- 첫번째 매개값: 에러 코드 (사용자 정의 예외 -20000 ~ -20999까지 사용 가능)
            -- 두번째 매개값: 에러 메세지
            -- OTHER 사용가능 다른 에러들
