...
DECLARE
    ...
    v_count NUMBER := 1; --begin
BEGIN
    WHILE v_count <= 10 --end -- WHILE 문은 변수를 꼭 선언해야 한다.
    LOOP
        EXIT WHEN v_count = 5;
...
BEGIN
    ...
    LOOP
        IF MOD(i, 2) = 0 THEN --MOD 나머지 구하는 함수
            dbms_output.put_line('구구단 '|| i || '단');
            FOR ii IN 1..9
            LOOP
                dbms_output.put_line(i || ' x ' || ii || ' = ' || ii*i);
            END LOOP;
...
BEGIN
    ...
    LOOP
        INSERT INTO board 
        VALUES(b_seq.NEXTVAL, 'test'||v_num, 'title'||v_num); -- NEXTVAL 시퀀스의 다음번호 조회
        v_num := v_num + 1;
    END LOOP;
    COMMIT;
END;
