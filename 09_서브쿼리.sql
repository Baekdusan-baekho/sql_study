...
WHERE salary > ANY (SELECT salary FROM employees
                 WHERE first_name = 'David');
-- 4800, 9500, 6800 보다 크다(4800) 1개 만족
...
WHERE salary > ALL (SELECT salary FROM employees
                 WHERE first_name = 'David');
-- 4800, 9500, 6800 보다 크다(9500) 모두 만족
...
-- 1은 상직적 의미 행의 길이만큼 1을 반환 행의 길이가 중요
-- 존재유무 파악 조회가 됨으로써 그 조건에 맞는 행이 있는가?

SELECT * FROM employees
WHERE EXISTS (SELECT 1 FROM departments
                WHERE department_id = 10);
-- department_id = 10 존재하면 true = * 모든 데이터 표현

------------------------------------------------------

-- SELECT 절에 서브쿼리를 붙이기.
-- 스칼라 서브쿼리 라고도 칭합니다.
-- 스칼라 서브쿼리: 실행 결과 단일 값을 반환하는 서브쿼리,
...
-- 특정 테이블 전체가 아닌 SELECT를 통해 
-- 일부 데이터를 조회한 것을 가상 테이블로 사용하고 싶을 때.
