...
SELECT TO_CHAR(20000, '9999') FROM dual;-- #은 표현할 수 없다는 뜻
SELECT TO_CHAR(20000, '999999999999') FROM dual; -- 20000앞에 빈공간 생김
SELECT TO_CHAR(20000.29, '99999.9') FROM dual; -- 소수점 1자리까지 표기
...
-- NULL 형태를 변환하는 함수 NVL(컬럼, 변환한 타겟값)
-- 타겟값은 항상 정수여야 한다.
-- 컬럼은 NULL인 값들만 포함함
...
SELECT
    first_name,
    NVL(commission_pct, 0) AS COMM_PCT--만약 NULL이 있다면 0으로 바꿔라
FROM employees;

-- NULL 변환 함수 NVL2(컬럼, null이 아닐 경우의 값, null일 경우의 값)
-- 문자열 문자열, 정수 정수, 문자열 정수 이렇게 가능
SELECT
    NVL2(NULL, '널아님', '널임')
FROM dual;
...
FROM employees
WHERE ((sysdate - hire_date)/365) >= 17 -- 근속년수>= 17 하면 안됨
ORDER BY 근속년수 DESC;
-- SQL 실행 순서
-- FROM -> WHERE -> SELECT -> ORDER BY 
-- 이기 때문에 SELECT에서 만들어지면 
-- WHERE에는 아직 없다 그래서 안됨
